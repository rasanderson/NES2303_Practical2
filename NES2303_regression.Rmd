...
library(mosaic)
library(ggpubr)
caterpillars <- read.csv("www/regression.csv")
...
between caterpillar growth rates and the amount of tannins in their
diet. Tannins are produced by some plants to reduce attack by insects. They are
what gives black tea its slightly bitter taste. The caterpillars were fed diets
that contained different amounts of tannins (mg), and the caterpillar growth 
...
`caterpillars`. It is useful to plot these before proceeding.
...
in the `caterpillars` dataframe. Click the `hint` button if you get stuck.
...
Now produce a scatterplot of the caterpillar growth rate (y-axis)
...
gf_point(growth ~ tannin, data=caterpillars) %>% 
...
question("Which statements below are true?",
         ...
         answer("Tannins inhibit caterpillar growth rate", correct = TRUE),
...
For our caterpillar growth vs tannins dataset, our null hypothesis $H0$ would be
"Tannins have no affect on caterpillar growth rate". Our linear model will allow
...
`caterpillar` data, making sure that you specify the response and
explanatory variables the right way round. Store the results in an R
objected called `caterpillar_lm` then use the `summary()` function to
...
```{r lm_calc-solution}
caterpillar_lm <- lm(growth ~ tannin, data=caterpillars)
summary(caterpillar_lm)
```
...
-   `r round(caterpillar_lm$coefficients[1],2)` for the intercept
-   `r round(caterpillar_lm$coefficients[2],2)` for the gradient. This
...
caterpillar growth rate and tannin". Basically, the **lower** these
...
79% of the variation in caterpillar growth can be explained by variation
in tannin.
...
Our linear model indicated that the relationship between tannins and caterpillar growth rate is
...
`gf_lm(growth ~ tannin, data=caterpillars)`
...
`caterpillar_lm$fitted.values` after you have created your linear model.
...
caterpillar_lm <- lm(growth ~ tannin, data=caterpillars)
model_fitted_values <- caterpillar_lm$fitted.values
model_fitted_values
```
...
caterpillar_lm <- lm(growth ~ tannin, data=caterpillars)
predictor <- makeFun(caterpillar_lm)
predictor(4.36)
predictor(1:10)
```
...
growth_predictor <- makeFun(caterpillar_lm)
...
caterpillar_lm <- lm(growth ~ tannin, data=caterpillars)
fits <- caterpillar_lm$fitted.values
...
`lm(growth ~ tannin, data=caterpillars)`
...
function; try it here to look at the numbers from your `caterpillar_lm`
...
`caterpillar_lm` as its parameter to display the positive and negative
residual values.
:::
...
caterpillar_lm <- lm(growth ~ tannin, data=caterpillars)
gf_histogram(~ residuals(caterpillar_lm)) %>% 
  gf_dens()
```
...
```{r normal_random_with_qqplot, warning=FALSE}
my_data <- rnorm(200) # Creates 50 data points from a random distribution
...
caterpillars when you only have 9 data points? Try it now:

```{r caterpillar_qqplot, exercise=TRUE, exercise.completion=FALSE}
caterpillar_lm <- lm(growth ~ tannin, data=caterpillars)

```

```{r caterpillar_qqplot-solution}
...
model_error <- residuals(caterpillar_lm)
gf_qq( ~ model_error) %>% 
  gf_qqline()
```
...
models, to determine whether caterpillar diet had any impact on caterpillar
...
`summary(caterpillar_lm)` function as a direct copy-and-paste to create Table 1.
