...
# Introduction
In this practical you will be introduced to **linear models** which provide a way to assess the relationships between a **response** variable and one or more **explanatory** variable(s). You will recall that we are trying to use a "goal-oriented" approach where possible in NES2504:
...
<center>
<h2><strong><span class="boxed">lm</span> ( <span class="boxed">y</span> ~ <span class="boxed">x</span> , data = <span class="boxed">mydata</span>, ... )</strong> 
</h2></center>

&nbsp;

**Important**
Commonly used names in textbooks for linear models like this differ depending on the explanatory variable. We make life easier by using the `lm()` function for both types of analysis:
...
[linear models with continuous explanatories (right-click and open in a new tab)](https://naturalandenvironmentalscience.shinyapps.io/linear_explan/) and  [linear models with categorical explanatories (right-click and open in a new tab)](https://naturalandenvironmentalscience.shinyapps.io/categorical_explan/). Please work through these websites **before** attempting this practical. You will also have the opportunity to gain confidence using linear models with a new dataset. **Specific objectives** are to:
...
| Benthic invertebrate diversity | Depth to sea floor | Continuous |
...
* Pollinator diversity and benthic invertebrate diversity are both continuous. It is reasonable to assume that the biodiversity of wildflowers will have a big effect on the pollinators out there, so wildflower diversity is shown as the explanatory variable. However, some scientists have shown that if the pollinator biodiversity declines sharply, this has a negative effect on the biodiversity of some wildflowers. So in this example the response and explanatory terms are a little more ambiguous.
...
2. Use File Explorer to copy `caterpillars.csv` from your Downloads folder to your NES2504 folder
3. Start RStudio. Select **File -> Open project...** and navigate to the RStudio project you created in the first practical session. This should be a file called NES2504; open it and when RStudio starts **check that `NES2504` is showing in the top right** corner of the RStudio window. This is **ESSENTIAL!** If you are unsure if you have opened your NES2504 project you **must** check with a demonstrator before proceding.
...
2. Now start writing the contents of your script, with a title line, explanation of what your script is doing, etc. **Note**: some of you are storing all your CSV files in a `Data` subfolder, others are storing them in the main `NES2504` folder. It does not matter which one you choose, but adjust the `read.csv()` call below accordingly:
...
If RGui or RStudio does not find the file `caterpillars.csv` check that you correctly copied the CSV from Downloads to NES2504, that there are no spelling errors, or whether you need to omit the `Data/` if you are not using a subfolder called `Data` within the `NES2504` folder.
...
It is essential to plot your data, to gain an understanding of its characteristics, before running a linear model. As you have two fairly simple variables, a scatter plot, also known as a **"point plot"**, with our explanatory variable of tannin on the x axis and growth on the y axis is required.  So thinking about our "goals" here:

&nbsp;

<center>
<h2><strong><span class="boxed">scatterplot</span> ( <span class="boxed">y</span> ~ <span class="boxed">x</span> , data = <span class="boxed">mydata</span>)</strong> 
</h2></center>

&nbsp;

which for your caterpillar data can be considered as:

&nbsp;

<center>
<h2><strong><span class="boxed">gf_point</span> ( <span class="boxed">growth</span> ~ <span class="boxed">tannin</span> , data = <span class="boxed">caterpillar_dat</span>)</strong> 
</h2></center>

&nbsp;


We now need to translate this goal into R via the following steps:

* In the RStudio code editor (top left of RStudio), at the bottom of your `Practical1.R` **enter a blank line**, then a line that says `# 3.2 Plot caterpillar data ####`. This makes your code easier to read and helps you navigate with a table of contents.
...
* Add a second row to your R code with command `gf_labs()` to label the x- and y-axes as shown below. Us `%>%` at the end of the preceding line so that the gf_labs() command is actioned. Remember that `%>%` indicates to R to continue adding features to your plot
...
The syntax with the `lm()` function is the same, as the `lm()` function is flexible and can be used for regression analysis:

&nbsp;

<center>
<h2><strong><span class="boxed">lm</span> ( <span class="boxed">growth</span> ~ <span class="boxed">tannin</span> , data = <span class="boxed">caterpillar_dat</span>)</strong> 
</h2></center>
...
Now some answers (please think about the above questions before reading these!!):
 ...
 + You may know that R-squared ($R^2$) is the proportion of variation explained by your linear model. What is a little confusing is that two $R^2$ values are given, one called `Multiple` and the other `Adujusted`. Always refer to the `Adjusted R-squared` which is `0.815` i.e. 81.5% of the variation in caterpillar growth rate can be explained by tannin
 ...
 + Overall model significance is shown by the F-statistic. The higher the F-statistic, the greater the "significance" of the model, which is reflected by a **lower** p-value. You would write something like:
 
 "Overall there was a significant negative relationship between caterpillar growth and tannin ($F_{1,48}$=216.9,p<0.001)".
 
**Note**: by convention you provide the **degrees of freedom as subscripts** when reporting F-values, and **3 decimal places** when reporting p-values.
...
You may find it useful to look at this <a href="https://naturalandenvironmentalscience.shinyapps.io/how_anova_works/#section-variances-with-continuous-explanatory-variables" target="_blank">Interactive demonstration (right-click and open in a new tab)</a> of the tannin and caterpillar growth data. It first shows you how sums of squares (SS) are calculated, then (scroll down) you can tweak the "noise" in the data to see what happens.
...
<a href="https://naturalandenvironmentalscience.shinyapps.io/Other_tests/#section-t-distribution" target="_blank">t-statistics (right-click and open in a new tab)</a>
. The t-distribution is like a fat-tailed normal distribution, and of particular value is the one-sample t-test. This checks whether an estimated value, here your Intercept and slope, is different to zero. If the p-value is **low** then there is a **high probability** that your estimate is **not** zero.
...
All models are simplifications of reality, and thus make various assumptions about your data. You will remember from the online interactive website tutorials
<a href="https://naturalandenvironmentalscience.shinyapps.io/NES2303_regression/#section-checking-model-assumptions" target="_blank">Checking model assumptions (right click and open in a new tab)</a>
 that one of the key assumptions of linear models is that the residuals are from a normal distribution. Residuals are the "noise" in your data. In a 'perfect' set of data all your observations would fall exactly on the fitted line. But look at the graph with your fitted line: only one observation (tannin concentration 7.0) lies very close to the fitted line. The others are all slightly above or below the line. A normal distribution is a bell-shaped curve, so if the model is a good one, if you create a frequency histogram of the residuals you should see (roughly) that sort of curve. **Note**: "residuals" are sometimes referred to as "errors" and represented by the Greek letter Epsilon ($\epsilon$).
...
Both these "diagnostic" plots can be created directly from your model output. R actually produces four diagnostic plots, but we are only interested in the first two, using the `which = c(1,2)` option. **Note:** After the first plot has displayed you will have to hit the **Enter** key in the RStudio Console (bottom left) to display the second plot:
...
* The columns headed `lwr` and `upr` give the lower and upper 95% confidence intervals of this difference. See
<a href="https://naturalandenvironmentalscience.shinyapps.io/variation/#section-confidence-intervals" target="_blank">Confidence intervals (right-click and open in a new tab)</a>
 on the Canvas website if you need a refresher
...
gf_boxplot(soil_resid ~ crop_growth$soil) %>%
  gf_jitter(soil_resid ~ crop_growth$soil) %>% 
  gf_labs(x = "Soil type", y = "Model residuals") %>% 
  gf_theme(theme_classic())
```

# Take home messages from today's practical
You have covered a lot of material in this practical, but the key thing to remember is that linear models provide a flexible wasy of thinking about lots of different 'conventional' statistical tests. "One-way ANOVA", "Regression" in some menu-driven statistics packages have different ways for the user to access them, which causes confusion. The advantage of R is that the same function, `lm()`, can be used for both. Some points to always remember though:

* Make sure you have your response and explanatory the correct way round, especially where both are continuous. It is easy in some 'ambiguous' biological or marine field surveys to code them incorrectly.
* Regression-style analyses have a continuous explanatory
* ANOVA-style analyses have a categorical explanatory.
* Watch your data codings for categorical explanatory. "A", "A", "A", "B", "B", "B" then R will be fine. But if you have "1", "1", "1", "2", "2", "2" etc. then R might incorrectly assume you are doing a regression-style analysis, and calculate insufficient coefficients for your dataset.

# Working with large biological datasets (complete in your own time)
Modern biology, zoology and marine bioscience generates big datasets that at first glance can be challenging to understand. Luckily, you can use the skills you've just learnt to summarise, visualise and analyse them. We will now do this with a dataset of over 300 observations!

The data are morphological measures of three species of Antartic penguins, although we will initially focus on their body mass. Key things for you to answer:
...
Download the `penguins.csv` data set from Canvas and copy it to your `NES2504` or `NES2504/Data` folder. If you want, open it in Microsoft Excel to see what it contains. If you have not already done so, you may find it helpful look at
<a href="https://naturalandenvironmentalscience.shinyapps.io/Summary_vis/" target="_blank">Interactive website on how to visualise data (right-click and open in a new tab)</a>
 which uses this dataset as an example.
...
## Interpretation
How might you write up these results in a report? Compare your ANOVA and Tukey results to your boxplot. Also look at this section of the 
<a href="https://rasanderson.github.io/NES2504-AnalysisHandbook/reports.html" target="_blank">NES2504 Data Analysis Handbook (right-click and open in a new tab)</a> on easy approaches to report-writing.
