...
* Start RStudio. Select **File -> Recent projects -> NES2303** to open up the RStudio project you created in the first practical session.
...
Now start writing the contents of your script, with a title line, explanation of what your script is doing, etc. **Note**: some of you are storing all your CSV files in a `Data` subfolder, others are storing them in the main `NES2303` folder. It does not matter which one you choose, but adjust the `read.csv()` call below accordingly:

**Note**: Write a suitable comment after a `#` symbol at the top of your script to explain what it is going to do. I would recommend putting "section numbers" that match those in this schedule to help you navigate your code. If you add `####` at the end of a comment you can add it to your Table of Contents (ask if unsure).
...
The residuals above and below the line should average around zero, and produce a rough bell-shaped curve.
...
In the plot headed 'Residuals vs Fitted' you can see that the size of the residuals is roughly the same for the full range of predicted values. The Normal-QQ plot  shows a rough straight line between the ranked expected and observed residuals. If the line formed an S-shape then your model would not be valid. Personally I find the QQ plot the most useful diagnostic plot to check that your model assumptions have been satisfied.
