...
Now some answers (please think about the above questions before reading these!!):
 ...
 + You may know that R-squared ($R^2$) is the proportion of variation explained by your linear model. What is a little confusing is that two $R^2$ values are given, one called `Multiple` and the other `Adujusted`. Always refer to the `Adjusted R-squared` which is `0.815` i.e. almost 79% of the variation in caterpillar growth rate can be explained by tannin
